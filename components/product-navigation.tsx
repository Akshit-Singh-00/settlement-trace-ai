...
export function ProductNavigation({ view, ready, section, onNavigate }: { view: AppView; ready: boolean; section: string; onNavigate: (target: string) => void }) {
  ...
  return <header className="site-header">
    ...
    <motion.div className="header-tools" initial={{ opacity: 0, x: reduced ? 0 : 16 }} animate={ready ? { opacity: 1, x: 0 } : { opacity: 0 }} transition={{ duration: 0.4, delay: 0.1, ease: flowEase }}>
      {/* A document navigation is required here: Vinext client routing fails at the authenticated workspace boundary. */}
      {/* eslint-disable-next-line @next/next/no-html-link-for-pages */}
      <a className="github-link" href="/workspace">Team workspace <ArrowUpRight size={13} /></a><a className="github-link" href="https://github.com/Akshit-Singh-00/settlement-trace-ai" target="_blank" rel="noreferrer">GitHub <ArrowUpRight size={13} /></a><ThemeToggle />
    </motion.div>
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent className="mobile-drawer" side="right">
        ...
        <nav aria-label="Mobile navigation">
          {/* eslint-disable-next-line @next/next/no-html-link-for-pages -- Use document navigation for the authenticated workspace. */}
          <a href="/workspace">Team workspace <ArrowUpRight size={16} /></a>
